...
function preload(){
  // sounds
  scoreSound = loadSound('music/score.mp3');
  wall_hitSound = loadSound('music/wall_hit.mp3');
  hitSound = loadSound('music/hit.mp3');
}
...
function draw() {  
  //fill the computer screen with orange color
  background("orange");
  White();
...
function White() {
  fill("white")
  rect(userPaddle.x - 5, userPaddle.y - 35, userPaddle.width, userPaddle.height);
  userPaddle.visible = false;

  rect(computerPaddle.x - 5, computerPaddle.y - 35, computerPaddle.width, computerPaddle.height);
